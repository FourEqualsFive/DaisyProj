...
void cr808bass::Init(float sample_rate)
{
...
inline float cr808bass::Diode(float x)
{
    if(x >= 0.0f)
    {
        return x;
...
void cr808bass::Process(float* sig, size_t size, bool trigger)
{
...
void cr808bass::SetSustain(bool sustain)
{
    sustain_ = sustain;
}

void cr808bass::SetAccent(float accent)
{
    accent_ = fclamp(accent, 0.f, 1.f);
}

void cr808bass::SetFreq(float f0)
{
    f0 /= sample_rate_;
    f0_ = fclamp(f0, 0.f, .5f);
}

void cr808bass::SetTone(float tone)
{
    tone_ = fclamp(tone, 0.f, 1.f);
}

void cr808bass::SetDecay(float decay)
{
    decay_ = decay * .1f;
    decay_ -= .1f;
}

void cr808bass::SetAttackFmAmount(float attack_fm_amount)
{
    attack_fm_amount_ = attack_fm_amount * 50.f;
}

void cr808bass::SetSelfFmAmount(float self_fm_amount)
{
